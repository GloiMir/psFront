...
import { loadSongsAdmin,loadSongs,setTitre,setPast } from '../redux/actions'
import { baseUrl } from '../api'

import NetInfo from '@react-native-community/netinfo'
let timer
const GestionComp = (props) => {
  ...
  const deleteSong = () =>{
    NetInfo.addEventListener((info)=>{
      if(info.isConnected===true){
        axios.post(`${baseUrl}/delete?id=${id}&type=composition`)
        .then((resp)=>{
          axios.get(`${baseUrl}/songs?type=composition`)
          .then(res=>{
            ...
            return(
              <View key={index} style={{flexDirection:'row',width:Dimensions.get('window').width,height:Dimensions.get('window').height/12,backgroundColor:'rgba(63,67,89,1)',marginTop:2}} >
                ...
                <View style={{flexDirection:'row',alignItems:'center',width:'20%',height:'100%',justifyContent:'space-around'}}>
                    <TouchableOpacity onPress={()=>props.navigation.navigate('EditSong',{data:{...item,type:'composition'}})}>
